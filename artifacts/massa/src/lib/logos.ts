...
// Popular MCP connectors offered when adding a new server. Kept as its own
// object so the Add Server picker can iterate and pre-warm their logos (see
// CURATED_MCP_CONNECTORS) without re-listing every brand name by hand.
const MCP_CONNECTOR_LOGOS: Record<string, LogoInfo> = {
...
export const COMPANY_LOGOS: Record<string, LogoInfo> = {
  ...
  // --- Popular MCP connectors (defined in MCP_CONNECTOR_LOGOS) ---
  ...MCP_CONNECTOR_LOGOS,
}

// Names of the curated MCP connectors offered in the Add Server picker, used to
// pre-warm their logos when the form opens so they render instantly.
export const CURATED_MCP_CONNECTORS = Object.keys(MCP_CONNECTOR_LOGOS)
